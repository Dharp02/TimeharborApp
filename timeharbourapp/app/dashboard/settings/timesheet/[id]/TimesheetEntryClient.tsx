...
/* ── component ─────────────────────────────────────────── */
export default function TimesheetEntryClient({ entryIdProp }: { entryIdProp?: string } = {}) {
  const router = useRouter();
  const params = useParams();
  const entryId = entryIdProp || (params.id as string); // Format: {sessionId}-in or {sessionId}-out
