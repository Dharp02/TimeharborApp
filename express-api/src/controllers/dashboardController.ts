...
ockInId) activeEventIds.add(openClockInId);
...
export const getMemberActivity = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // ---------------------------------------------------------------------------
    // Time tracking totals — use UserDailyStat (same source as the Timesheet page
    // and getDashboardStats) so leader-view and member-view are always identical.
    // ---------------------------------------------------------------------------
    const localDate = (d: Date) =>
      `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
    const today = localDate(now);
    const startOfWeekDate = localDate(startOfWeek);
    const startOfMonthDate = localDate(startOfMonth);

    const statWhere: any = {
      userId: memberId,
      date: { [Op.gte]: startOfMonthDate },
    };
    if (teamId) statWhere.teamId = teamId as string;

    const statRows = await UserDailyStat.findAll({
      where: statWhere,
      attributes: ['date', 'totalMs'],
    });

    let todayDuration = statRows
      .filter(r => r.date === today)
      .reduce((acc, r) => acc + Number(r.totalMs), 0);

    let weekDuration = statRows
      .filter(r => r.date >= startOfWeekDate)
      .reduce((acc, r) => acc + Number(r.totalMs), 0);

    let monthDuration = statRows.reduce((acc, r) => acc + Number(r.totalMs), 0);

    // Inject live time for any currently-active session (same logic as getDashboardStats / getTimesheetTotals)
    const lastClockEventWhere: any = { userId: memberId, type: { [Op.in]: ['CLOCK_IN', 'CLOCK_OUT'] } };
    if (teamId) lastClockEventWhere.teamId = teamId as string;
    ...
    if (lastClockEvent?.type === 'CLOCK_IN') {
      ...
      const liveMs = Math.max(0, now.getTime() - sessionStartMs - totalBreakMs);
      if (liveMs > 0) {
        const sessionDate = localDate(new Date(lastClockEvent.timestamp));
        if (sessionDate === today) todayDuration += liveMs;
        if (sessionDate >= startOfWeekDate) weekDuration += liveMs;
        monthDuration += liveMs;
      }
    }

    // Fetch today's clock events for clockTimes (used in timeTracking.today.clockEvents)
    const todayEntries = await WorkLog.findAll({
      where: {
        userId: memberId,
    ...
    // todayEntries is already filtered to CLOCK_IN/CLOCK_OUT
    const todayClockEvents = todayEntries;
